...
export default function LoginDetails() {
    ...
    return (
        <View style={{ width: '100%', height: height, backgroundColor: '#0a3d62', alignItems: 'center' }}>
            ...
            <TouchableOpacity
                disabled={first.length == 0 || last.length == 0 || email.length == 0} activeOpacity={0.8} onPress={handlePress}
                style={{ marginTop: 50, height: 50, width: '90%', backgroundColor: first.length == 0 || last.length == 0 || email.length == 0 ? '#4b4b4b' : '#474787', justifyContent: 'center', alignItems: 'center', borderRadius: 7 }}>
                <Text style={{ fontSize: 20, fontWeight: 500, color: first.length == 0 || last.length == 0 || email.length == 0 ? '' : '#ffffff' }}>Create Account</Text>
            </TouchableOpacity>
        </View>
    )
}